...
DEPLOY_SUCCESS_LOG_PATTERN='WFLYSRV0009|WFLYSRV0010|WFLYSRV0011|WFLYSRV0016|WFLYSRV0027|WFLYDR0001|WFLYDS0010|deploy'
...
show_datasource_diagnostics_from_logs() {
  ...
  logs="$1"
  ds_names="$(printf '%s\n' "$logs" | grep -Ei "$DATASOURCE_SUCCESS_LOG_PATTERN" | grep -Eo '\[java:[^][:space:]]*\]|java:[^][:space:]"]+' | tr -d '[]' | sort -u || true)"
  ds_errors="$(
    printf '%s\n' "$logs" \
