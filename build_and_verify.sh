...
STARTUP_IMPORTANT_LOG_PATTERN='WFLYSRV00(25|26)|WFLYCTL0183|WFLYCTL0448|JBoss EAP.*started in'
...
DEPLOY_ERROR_LOG_PATTERN='WFLYCTL0013|WFLYCTL0080|WFLYSRV0026|deployment.*(failed|error)|ERROR|Exception'
...
show_deploy_logs() {
  ...
  path_logs="$(printf '%s\n' "$success_logs" | grep -E '/[^[:space:]"]+' | tail -n "$DEPLOY_LOG_LINES" || true)"
