...
DATASOURCE_ERROR_SCOPE_PATTERN='datasource|data source|JNDI|java:|WFLYJCA|IJ[0-9]{6}|jboss\.naming\.context\.java'
DATASOURCE_ERROR_DETAIL_PATTERN='warn|error|fail|failure|exception|missing|unavailable|not installed|unable|WFLYCTL0013|WFLYCTL0080|IJ[0-9]{6}'
...
show_datasource_diagnostics_from_logs() {
  ...
  ds_errors="$(
    printf '%s\n' "$logs" \
      | grep -Ei "$DATASOURCE_ERROR_SCOPE_PATTERN" \
      | grep -Ei "$DATASOURCE_ERROR_DETAIL_PATTERN" \
