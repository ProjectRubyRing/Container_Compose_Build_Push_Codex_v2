...
assert_matches "$FAKE_DOCKER_CALLS" 'exec cid-app find / .* -path /afs .* -path /local/aws-cli .* -path /opt/jboss-eap/modules/system/layers/base .* -path /share .* -path /usr/lib64 .* -prune -print0 -o -type d -print0'
assert_matches "$FAKE_DOCKER_CALLS" 'exec cid-app find / .* -path /afs .* -path /local/aws-cli .* -path /opt/jboss-eap/modules/system/layers/base .* -path /share .* -path /usr/lib64 .* -prune -o -type f -print0'
...
assert_not_contains "$full_report" "share-hidden/"
...
assert_not_contains "$full_report" "share-hidden.txt"
