...
assert_contains "$success_output" "X11/"
assert_contains "$success_output" "doc/"
assert_contains "$success_output" "icons/"
assert_contains "$success_output" "licenses/"
assert_contains "$success_output" "man/"
assert_contains "$success_output" "osinfo/"
assert_contains "$success_output" "public/"
assert_contains "$success_output" "zoneinfo/"
assert_contains "$success_output" "[ファイル] readme.txt"
...
assert_matches "$FAKE_DOCKER_CALLS" 'exec cid-app find / .* -path /afs .* -path /local/aws-cli .* -path /opt/jboss-eap/modules/system/layers/base .* -path /share/X11 .* -path /share/doc .* -path /share/icons .* -path /share/licenses .* -path /share/man .* -path /share/osinfo .* -path /share/zoneinfo .* -path /usr/lib64 .* -prune -print0 -o -type d -print0'
assert_matches "$FAKE_DOCKER_CALLS" 'exec cid-app find / .* -path /afs .* -path /local/aws-cli .* -path /opt/jboss-eap/modules/system/layers/base .* -path /share/X11 .* -path /share/doc .* -path /share/icons .* -path /share/licenses .* -path /share/man .* -path /share/osinfo .* -path /share/zoneinfo .* -path /usr/lib64 .* -prune -o -type f -print0'
...
assert_contains "$full_report" "X11/"
assert_contains "$full_report" "doc/"
assert_contains "$full_report" "icons/"
assert_contains "$full_report" "licenses/"
assert_contains "$full_report" "man/"
assert_contains "$full_report" "osinfo/"
assert_contains "$full_report" "public/"
assert_contains "$full_report" "assets/"
assert_contains "$full_report" "zoneinfo/"
assert_contains "$full_report" "readme.txt"
assert_contains "$full_report" "logo.svg"
